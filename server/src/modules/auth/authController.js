...
export async function login(req, res) {
    try {
        ...
        const user = await models.User.findOne({
            attributes: ['id', 'email', 'fullName', 'birthday', 'phone', 'password', 'gender'],
            where: {
                email,
            },
        });
...
export async function refreshToken(req, res) {
    try {
        const { loginUser = {},  oldRefreshToken = null } = req;
        ...
        const isTokenExist = await checkIfTokenExist(user, oldRefreshToken);
        if (!isTokenExist) {
            return res.json(respondWithError(ErrorCodes.ERROR_CODE_UNAUTHORIZED, 'Unauthorized'));
        }
        const { token, rToken, permissions } = await signToken(user);
        await Promise.all([destroyToken(isTokenExist.id), saveToken(user, rToken)]);
